<?php	    
    ...
    $queryob = mysqli_query($connection,"SELECT sum(receiptcash) as rcash, sum(receiptadj) as radj, sum(paymentcash) as pcash, sum(paymentadj) as padj FROM acc_cashbook, acc_transactions
                            WHERE accno = '$duesid'  AND date < '$fromdate' AND  acc_cashbook.TransID = acc_transactions.TransID AND acc_transactions.TransStatus = 1 AND subheadid = 20 GROUP BY accno" ); 
    ...
    $finalquery = mysqli_query($connection,"SELECT * FROM acc_cashbook, acc_transactions
                            WHERE accno = '$duesid'  AND  acc_cashbook.TransID = acc_transactions.TransID AND acc_transactions.TransStatus = 1 AND subheadid = 20 AND date BETWEEN '$fromdate' AND '$todate' ORDER BY date"); 
