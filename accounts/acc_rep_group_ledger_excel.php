<?php    
    ...
    $finalq="select memid,".$cstring."    
        ...
        from (SELECT memid,max(subheadid) AS subhead,SUM(receiptcash) AS receipt,SUM(paymentcash) AS payment FROM acc_cashbook, acc_transactions 
                WHERE acc_cashbook.TransID = acc_transactions.TransID AND acc_transactions.TransStatus = 1 AND groupid = '$group' AND subheadid != 14 AND LEFT(date,7) = '$month1' GROUP BY subheadid,memid) AS A) AS B GROUP BY memid) AS V";
    ...
    $output.= "<table style='font-family:verdana; font-size:14px;'>
                <tbody>
                    <tr>";
    $output.= "<td colspan='1'>Cluster Name: ".$clustername."</td></tr>";
    $output.= "<tr><td colspan='1'>Group Name: ".$groupname."</td></tr><tbody></table>";
