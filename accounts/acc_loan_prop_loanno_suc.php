...
include("accounts_session.php");
    ...
    if(isset($_GET['id'])){
        ...
        $memid = $newloan['memid'];
        $oldloan = $newloan['loanno'];
        $subheadid = $newloan['subid'];
        $bankheadid = $newloan['banksubid'];
        $loan = $newloan['proposedloan'];
        $balance = $newloan['loanbalance'];
        $clusterpropid = $newloan['ClusterPropID'];
        $loanins = $newloan['proposedins'];
        $total = $loan + $balance;
    ...
    if($balance == 0){
        $cashbookloan = mysqli_query($connection,"INSERT INTO acc_cashbook (`date`, `clusterid`, `groupid`, `TransID`, `memid`, `subheadid`, `accno`, `details`, `paymentadj`,`remarks`,`entryempid`,`timedate`)
                                VALUES('$doi','$clusterid','$groupid', '$transid', '$memid', '$subheadid', '$loanid', 'New Loan','$loan','New Loan','$user','$timedate')");
        
        $cashbookbank = mysqli_query($connection,"INSERT INTO acc_cashbook (`date`, `clusterid`, `groupid`, `TransID`, `memid`, `subheadid`, `accno`, `details`, `receiptadj`,`remarks`,`entryempid`,`timedate`)
                                VALUES('$doi','$clusterid','$groupid', '$transid', '$memid', '$bankheadid', '$loanid', 'New Loan','$loan','New Loan','$user','$timedate')");
    }
    else{
        $cashbookloanclose = mysqli_query($connection,"INSERT INTO acc_cashbook (`date`, `clusterid`, `groupid`, `TransID`, `memid`, `subheadid`, `accno`, `details`, `receiptadj`,`remarks`,`entryempid`,`timedate`)
                                VALUES('$doi','$clusterid','$groupid', '$transid', '$memid', '$subheadid', '$oldloan', 'Loan Closed','$balance','Loan Closed','$user','$timedate')");

        $cashbookloan = mysqli_query($connection,"INSERT INTO acc_cashbook (`date`, `clusterid`, `groupid`, `TransID`, `memid`, `subheadid`, `accno`, `details`, `paymentadj`,`remarks`,`entryempid`,`timedate`)
                                VALUES('$doi','$clusterid','$groupid', '$transid', '$memid', '$subheadid', '$loanid', 'New Loan','$total','New Loan','$user','$timedate')");
    ...
    //Loan Ledger Start	
    if($balance == 0){
        $loantable = mysqli_query($connection,"INSERT INTO acc_loans (`memid`, `subheadid`, `loanno`, `dateofissue`, `ob`,`installment`, `cb`, `empid`)
                            VALUES('$memid','$subheadid','$loanid', '$doi', '$loan','$loanins', '$loan', '$user')");	
    }  
    else{
        $oldloanupdate = mysqli_query($connection,"UPDATE acc_loans SET cb =0, status = 0 WHERE loanno = '$oldloan'");
        $loantable = mysqli_query($connection,"INSERT INTO acc_loans (`memid`, `subheadid`, `loanno`, `dateofissue`, `ob`,`installment`, `cb`, `empid`)
                            VALUES('$memid','$subheadid','$loanid', '$doi', '$total','$loanins', '$total', '$user')");	
    }
    
    //Loan Ledger End  
    
    //loan dummy update
        $loandummy = mysqli_query($connection,"UPDATE acc_loan_dummy SET newloanno = '$loanid', TransID = '$transid' WHERE id = '$id'");
    //loan dummy End 
	...
	if($transinsert && $cashbookloanclose && $cashbookloan && $cashbookbank && $oldloanupdate && $loantable && $loandummy && $loanincome){
		mysqli_query($connection,"commit");
	}
	else if($transinsert && $cashbookloan && $cashbookbank && $loantable && $loandummy && $loanincome){
		mysqli_query($connection,"commit");
	}    
	else{
				...
				<div class="main-content-inner">					
					<div class="page-content">
						<div class="page-header">
							<h1>
								 New Loan Issued Successfully with Loan No: <?php echo $loanid." - Transid : ".$transid; ?> 
								<small>
                        ...
                        <div class="clearfix form-group">
                          <div class="col-md-offset-3 col-md-9">
                            <a href="acc_loan_prop_view.php?clustpropid=<?php echo $clusterpropid; ?>"><button class="btn btn-info" type="button">
