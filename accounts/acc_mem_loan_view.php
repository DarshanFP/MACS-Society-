...
$finalquery = "SELECT TransID,date1,".$cstring." 
            ...
            FROM (SELECT acc_cashbook.* FROM acc_cashbook, acc_transactions 
                                WHERE accno = '$loanno'  AND  acc_cashbook.TransID = acc_transactions.TransID AND acc_transactions.TransStatus = 1 ) AS A) AS B GROUP BY TransID) AS V WHERE date1 BETWEEN '$fromdate' AND '$todate' ORDER BY date1";    
...
if($count>0){
    ...
    $i = 0;
        while($row4 = mysqli_fetch_assoc($finalq)){
            if($slno == 1){
                $obsql = mysqli_query($connection,"SELECT (sum(`paymentcash`)+sum(`paymentadj`)-sum(`receiptcash`)-sum(`receiptadj`)) AS ob FROM `acc_cashbook` WHERE date < '$fromdate' AND accno = '$loanno' AND subheadid = 5 GROUP BY accno");
