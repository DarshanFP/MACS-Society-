<?php	    
  ...
  if(isset($_GET['memid'])){
        ...
        $installment  = mysqli_query($connection,"SELECT installment FROM acc_loans WHERE loanno = '$loanno'");
        $installment = mysqli_fetch_assoc($installment);
        $installment = $installment['installment'];
  
    //$sql2 = mysqli_query($connection,"SELECT SubHead,SubID FROM acc_loans, acc_subhead WHERE loanno = '$loanno' AND SubID = subheadid");
    //$row2 = mysqli_fetch_assoc($sql2);
    //$subid = $row2['SubID'];

    //$sql = mysqli_query($connection,"SELECT acc_cashbook.* FROM acc_cashbook, acc_transactions 
    //                                WHERE accno = '$loanno'  AND  acc_cashbook.TransID = acc_transactions.TransID AND acc_transactions.TransStatus = 1 ");
    ...
    $finalquery = "SELECT * FROM (SELECT rowid,TransID,date1,".$cstring." 
                FROM (SELECT min(id) AS rowid,TransID,min(date) as date1,".$sstring."
                FROM (SELECT A.*,".$casestring."
                FROM (SELECT acc_cashbook.* FROM acc_cashbook, acc_transactions 
                                    WHERE accno = '$loanno'  AND  acc_cashbook.TransID = acc_transactions.TransID AND acc_transactions.TransStatus = 1 ) AS A) AS B GROUP BY TransID) AS V ORDER BY date1 DESC,RIGHT(TransID,(CHAR_LENGTH(TransID)-3)) LIMIT 10) sub ORDER BY date1,RIGHT(TransID,(CHAR_LENGTH(TransID)-3)) ASC";    
									...
									<div id="user-profile-1" class="user-profile row">
										...
										<div class="col-xs-12 col-sm-10">											
											...
											<div class="profile-user-info profile-user-info-striped">
                                                ...
                                                <div class="profile-info-row">
													<div class="profile-info-name"> Loan No </div>

													<div class="profile-info-value">
														<span class="editable" id="loanno"> <?php echo $loanno;?> </span>
                                                ...
                                                <div class="profile-info-row">
                                                    ...
                                                    <div class="profile-info-name">Monthly Principal</div>
																	...
																	<table style="font-size:13px" id="simple-table" class="table  table-bordered table-hover">
																		<thead>
																			<tr>													
                                                                                ...
                                                                                <th class="center" width='12%'>Interest Received</th>
																		...
																		<?php 
                                                                            if($count>0){
                                                                                ...
                                                                                while($row4 = mysqli_fetch_assoc($finalq)){
                                                                                    if($slno == 1){
                                                                                        $rowid = $row4['rowid'];
                                                                                        $obsql = mysqli_query($connection,"SELECT (sum(`paymentcash`)+sum(`paymentadj`)-sum(`receiptcash`)-sum(`receiptadj`)) AS ob FROM `acc_cashbook` WHERE id < '$rowid' AND accno = '$loanno' AND subheadid = 5 GROUP BY accno");
